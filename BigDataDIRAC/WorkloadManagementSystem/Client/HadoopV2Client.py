...
shutil.rmtree( workingDirectory )
   ...
   self.log.debug( 'BigData Hadoop V.2 result OK', result )

   if not result['OK']:
     for getit in result['Value']:
       resulting = re.search( "job_+([^:]+)\n", str( getit ) )
       if ( resulting != None ):
  ...
  def delHadoopData( self, tempPath ):
    cmdSeq = "hadoop dfs -rm -r " + tempPath
  ...
  def jobStatus( self, jobId, user, host ):
    cmdSeq = "ssh -l " + user + " " + host + " 'hadoop job -status " + jobId + "'"

    gLogger.info( 'Command Submitted: ', cmdSeq )

    result = self.sshConnect.sshOnlyCall( 10, cmdSeq )
    if result['OK'] == True:
      for string in result['Value']:
        stringer = str( string )
        m = stringer.find( "SUCCEEDED" )
        if ( m != -1 ):
          return DIRAC.S_OK( "Succeded" )
        m = stringer.find( "RUNNING" )
        if ( m != -1 ):
  ...
  def newJob( self, path, jobDiracId, bdJobId ):
    ...
    cmdSeq = "ssh -l " + self.user + " " + self.publicIP + " /" + path + "/" + str( jobDiracId ) + "/BigDat_*_getInfo.py -c step1 | wc -l"
    gLogger.info( 'Command Submitted: ', cmdSeq )
    ...
    if returned != None:
      if ( returned['Value'][1] != "" ):
        if ( returned['Value'][1] > 1 ):
          cmdSeq = "ssh -l " + self.user + " " + self.publicIP + " /" + path + "/" + str( jobDiracId ) + "/BigDat_*_getInfo.py -c step1 | tail -n1"
          gLogger.info( 'Command Submitted: ', cmdSeq )
          returned = self.sshConnect.sshOnlyCall( 10, cmdSeq )
          gLogger.info( 'Command Submitted: ', returned )
          if returned != None:
...
#################################################################################################################
  def submitPilotJob( self, tempPath, jobXMLName, proxy ):
    ...
    executableFile = "sh -c " + executableFile + " $*  > " + tempPath + "/pilot.out 2> " + tempPath + "/pilot.err"
    self.log.verbose( "Executable file command: ", executableFile )
    result = self.sshConnect.sshCall( 10, executableFile )
